...
# Adjusted for the Exascale FastForward compile-time directive "EFF"
function_pattern = r'(?P<eff>(EFF)[ ]+)?(?P<mpi>(MPI)[ ]+)?(?P<version>([0-9]\.[0-9]+\.[0-9]+))?([ ]+)?(?P<code>(unsigned[ ]+)?[a-zA-Z_]+[a-zA-Z0-9_]*\**)[ ]+(?P<fname>[a-zA-Z_]+[a-zA-Z0-9_]*)[ ]*\((?P<sig>[a-zA-Z0-9_,* ]*)\)'
...
class FunctionCruncher2(object):
    ...
    def handle_line(self, line):
        ...
        if line.startswith(' '):
            ...
            if function_parts['eff'] is not None:
                function_parts['eff'] = True
    ...
    def add_cython_if(self, function_parts, block):
        ...
        if function_parts['eff']:
            block = wrapif('EFF', block)
